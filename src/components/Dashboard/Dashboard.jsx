...
import { getUser,getEvents,setUserFetching,clearState } from './DashboardSlice';
        ...
        <>
            <div className="w-8/12 mx-auto relative h-screen flex flex-col justify-start artist">
                ...
                >
                    <input
                        value={search}
                        className="relative top-0  w-full h-10 shadow py-3 pl-3 pr-10"
                        placeholder="Search Text"
                        onChange={handleSearch}
                    />
                    <div className="absolute right-3 top-2">
                        {userIsFetching?  <img key="122" src={loadingIcon} className="w-6 h-6 " />
                            : [search.length>0?<img key="12" src={CrossIcon} className="w-6 h-6 cursor-pointer" onClick={reset}/>:<svg
                                className="w-6 h-6 "
                                key="121"
                ...
                >
                    <div className='text-left pb-1'>
                        <p>Search Result for <strong>{search}</strong></p>
                    </div>
    
                    <div>
                        <div className="card-main w-full flex flex-wrap justify-between ">
                            <ProfileCard user={userData} artist={artist} onArtistSelect={selectArtist}></ProfileCard>
                        </div>
                    </div>
                </div>
                <div className={'w-full mx-auto relative h-screen flex flex-col justify-start event ' +
                (artist?.name && search.length > 0 ? 'transition-cards' : ' show-listing')
                }>
