...
export const getUser = createAsyncThunk('/artists', async (artistName) => {
...
export const getEvents = createAsyncThunk('/artists/events', async (data) => {
    ...
    if (data.length === 1) {
        ...
        const response = await axios.get(
            `/artists/${data[0]}/events?app_id=foo&date=${data[1]}`
        );
        return response.data;
    } else {
        const response = await axios.get(
            `/artists/${data[0]}/events?app_id=foo&date=${data[1]}%2C${data[2]}`
        );
        return response.data;
    }
});
